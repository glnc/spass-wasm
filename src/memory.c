...
POINTER memory_Realloc(POINTER Ptr, unsigned int OldSize, unsigned int NewSize) {  
...
#ifdef CHECK
POINTER memory_ReallocIntern(POINTER Ptr, 
...
#endif
/********************************************************
  INPUT  : <Ptr> is an allocated block of the memory module
           of size <OldSize> and <NewSize> is its new adjusted size
  RETURNS: A pointer to a block of <NewSize> bytes, old content copied.  
  EFFECT:  Potentially a new memory block is returned if <Ptr> cannot
           be extended    
...
*********************************************************/
{
  POINTER NewMem;
#ifdef CHECK
  NewMem = memory_MallocIntern(NewSize, File, Line);
#else
  NewMem = memory_Malloc(NewSize);
#endif
  memcpy(NewMem,Ptr,OldSize>NewSize?NewSize:OldSize);
#ifdef CHECK
  memory_FreeIntern(Ptr, OldSize, File, Line);
#else
