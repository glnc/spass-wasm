...
***************************************************************/
{
 ...
 int pcheck_MaxParentSplitLevel(CLAUSE Clause) 
/**************************************************************
  INPUT:   A clause
...
static intptr_t pcheck_LabelToNumber(const char* Label)
/**************************************************************
...
***************************************************************/
{
  return (intptr_t)Number - clause_Number(*(const CLAUSE*)ClausePtr);
}
...
***************************************************************/
{
  ...
  for (Position = 0; Position < Size; Position++)  {
    ...
    for (ScanParents = NewParents; !list_Empty(ScanParents); ScanParents = list_Cdr(ScanParents)) {     
      ParentNum = (intptr_t)list_Car(ScanParents);
...
***************************************************************/
{
  ...
  for (Position = 0; Position < Size; Position++)  {
	...
	ParentNum = (intptr_t)list_Car(Parents);
	PLitNum   = (intptr_t)list_Car(PLits);
	/* Binary search for parent clause with number <ParentNum>. */
	Parent = bsearch((const void*)ParentNum, ClauseVector, Size,
			 sizeof(CLAUSE), pcheck_CompareNumberAndClause);
	if (Parent == NULL) {
...
***************************************************************/
{
  ...
  while (!list_Empty(ProofRest)) {    
    ...
    Level         = (intptr_t)list_Fourth(ProofLine);
...
/* EK: unused, only recursive */
 void pcheck_TableauSplitsComplete(TABLEAU T)
/**************************************************************
  INPUT  : A tableau
...
/* EK: unused, only recursive */
 void pcheck_SplitFormats(TABLEAU T)
/**************************************************************
  INPUT  : A tableau
...
/* EK: unused, only recursive */
 void pcheck_SplitPrecheck(TABLEAU T)
/**************************************************************
  INPUT  : A tableau.
...
***************************************************************/
{
  ...
  if (!pcheck_Quiet) {
    fputs("removing unfinished splits...", stdout);
    fflush(stdout);
  }
  (*Tableau) = tab_RemoveUnfinishedSplits(*Tableau, &RedundantClauses);
  if (!pcheck_Quiet)
    puts("finished.");
...
/**************************************************************
  ...
  RETURNS: A list of clauses where incomplete splits
